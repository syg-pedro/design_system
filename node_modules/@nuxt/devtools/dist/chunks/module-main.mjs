...
import { d as defaultAllowedExtensions, a as defaultTabOptions } from '../shared/devtools.DClER4Dv.mjs';
...
function setupAnalyzeBuildRPC({ nuxt, refresh }) {
  ...
  async function startAnalyzeBuild(name) {
    ...
    initalized = void 0;
    promise = Promise.resolve(result.getResult()).then(() => {
      refresh("getAnalyzeBuildInfo");
...
function setupAssetsRPC({ nuxt, refresh, options }) {
  ...
  return {
    ...
    async writeStaticAssets(files, folder) {
...
You can configure it in Nuxt config at \`devtools.assets.uploadExtensions\`.`);
    ...
    async deleteStaticAsset(path) {
      return await fs.unlink(path);
    },
    async renameStaticAsset(oldPath, newPath) {
...
function setupRPC(nuxt, options) {
  ...
  function createBroadcastProxy(prefix = "") {
    return new Proxy({}, {
      get: (_, method) => {
        if (typeof method !== "string")
          return;
        const fullMethod = prefix ? `${prefix}:${method}` : method;
        const fn = (...args) => broadcast(fullMethod, ...args);
        fn.asEvent = (...args) => broadcast(fullMethod, ...args);
        return fn;
      }
    });
  }
    ...
    if (devtoolsKitCtx) {
      for (const [fnName, handler] of Object.entries(functions)) {
        if (typeof handler === "function") {
          devtoolsKitCtx.rpc.register({ name: `${namespace}:${fnName}`, handler });
        }
      }
    }
    return {
  ...
  function connectDevToolsKit(ctx2) {
    if (devtoolsKitCtx)
      return;
    devtoolsKitCtx = ctx2;
    const host = ctx2.rpc;
